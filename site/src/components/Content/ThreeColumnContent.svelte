...
if (!data.textBoxWidth || data.textBoxWidth === "default") {
    addedClasses = `prose-sm sm:prose sm:text-gray-500 md:prose-md md:text-gray-500 lg:prose-lg lg:text-gray-500 xl:prose-xl xl:text-gray-500`;
  } else {
...
<div class={`${data.backgroundColor === "faded" ? "bg-gray-50" : "bg-white"} relative py-16 overflow-hidden`}>
  <div class="container">
    ...
    <div class="grid gap-4 grid-cols-1 md:grid-cols-3 md:gap-6 lg:gap-8">
      ...
      <BlockContent content={data.column3} classes={`prose text-gray-500 mx-auto ${addedClasses}`} />
    </div>
  </div>
</div>